document.addEventListener("DOMContentLoaded", () => {
    ...
    if (moduleTitleDisplay) { // Check if we're on the module page
        ...
        const moduleEbookParam = params.get("ebook");
        const moduleVideoParam = params.get("video");
        let moduleVideos = [];
        let moduleEbook = null;
        ...
        if (moduleEbookParam) {
            try {
                moduleEbook = JSON.parse(decodeURIComponent(moduleEbookParam));
            } catch (e) {
                console.error("Error parsing ebook:", e);
            }
        }

        if (moduleVideoParam) {
            try {
                moduleVideo = JSON.parse(decodeURIComponent(moduleVideoParam));
            } catch (e) {
        ...
        if (moduleType === "combined" && moduleEbook && moduleVideo) {
            // Create combined content with tabs
            const combinedContent = document.createElement("div");
            combinedContent.className = "combined-content";
            combinedContent.innerHTML = `
                <div class="content-tabs">
                    <button class="tab-button active" data-tab="ebook">📖 Ebook: ${moduleEbook.title}</button>
                    <button class="tab-button" data-tab="video">🎥 Video: ${moduleVideo.title}</button>
                </div>
                <div class="tab-content">
                    <div id="ebookTab" class="tab-pane active">
                        <iframe src="${moduleEbook.src}" allowfullscreen></iframe>
                    </div>
                    <div id="videoTab" class="tab-pane">
                        <iframe src="${moduleVideo.src}?autoplay=0&rel=0" title="${moduleVideo.title}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                    </div>
                </div>
            `;

            // Hide original content areas
            ebookContentDiv.style.display = "none";
            videoAreaDiv.style.display = "none";

            // Insert combined content after the header
            const header = document.querySelector('.header-modulo');
            header.insertAdjacentElement('afterend', combinedContent);

            // Add tab functionality
            const tabButtons = combinedContent.querySelectorAll('.tab-button');
            const tabPanes = combinedContent.querySelectorAll('.tab-pane');
